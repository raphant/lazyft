...
class SolipsisSh(IStrategy):
    sh = SpaceHandler(__file__, disable=__name__ != __qualname__)
    # region Buy Space Hyperopt Variables

    # Base Pair Params
    buy_base_mp = IntParameter(
        10, 50, default=30, space="buy", load=True, optimize=sh.get_space("mp")
    )
    buy_base_rmi_max = IntParameter(
        30, 60, default=50, space="buy", load=True, optimize=sh.get_space("rmi")
    )
    buy_base_rmi_min = IntParameter(
        0, 30, default=20, space="buy", load=True, optimize=sh.get_space("rmi")
    )
    buy_base_ma_streak = IntParameter(
        1, 4, default=1, space="buy", load=True, optimize=sh.get_space("ma")
    )
    buy_base_rmi_streak = IntParameter(
        3, 8, default=3, space="buy", load=True, optimize=sh.get_space("rmi")
    )
    buy_base_trigger = CategoricalParameter(
        ...
        space="buy",
        load=True,
        optimize=sh.get_space("trigger"),
    )
    buy_inf_pct_adr = DecimalParameter(
        0.70, 0.99, default=0.80, space="buy", load=True, optimize=sh.get_space("adr")
    )
    # BTC Informative
    buy_xbtc_guard = CategoricalParameter(
        ...
        default="lazy",
        space="buy",
        optimize=sh.get_space("guard"),
    )
    buy_xbtc_base_rmi = IntParameter(
        20, 70, default=40, space="buy", load=True, optimize=sh.get_space("rmi")
    )
    # BTC / ETH Stake Parameters
    buy_xtra_base_stake_rmi = IntParameter(
        10, 50, default=50, space="buy", load=True, optimize=sh.get_space("rmi")
    )
    buy_xtra_base_fiat_rmi = IntParameter(
        30, 70, default=50, space="buy", load=True, optimize=sh.get_space("rmi")
    )
    # endregion
    # Sell Space Params are being used for both custom_stoploss and custom_sell

    # Custom Sell Profit (formerly Dynamic ROI)
    # region Sell Space Hyperopt Variables
    sell_roi_type = CategoricalParameter(
        ["static", "decay", "step"],
        default="step",
        space="sell",
        load=True,
        optimize=sh.get_space("custom_roi"),
    )
    ...
    sell_pullback = CategoricalParameter(
        ...
        space="sell",
        load=True,
        optimize=sh.get_space("pullback"),
    )
    ...
    sell_stop_loss_threshold = DecimalParameter(
        ...
        space="sell",
        load=True,
        optimize=sh.get_space("custom_stoploss"),
    )
    ...
    def custom_sell(
        ...
        **kwargs,
        ...
        ) and last_candle["candle-up-trend"] == 1:
            in_trend = True
        ...
        if in_trend and current_profit > 0:
            ...
            if self.sell_pullback.value and (current_profit <= pullback_value):
                if (
        ...
        # If we are not in a trend, just use the roi value
            ...
            if self.custom_trade_info[trade.pair]["had-trend"]:
                if current_profit > min_roi:
                    ...
                    return "trend_roi"
                elif not self.sell_endtrend_respect_roi.value:
                    self.custom_trade_info[trade.pair]["had-trend"] = False
                    return "trend_noroi"
