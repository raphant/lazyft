...
from indicatormix.entities import series as im_series
from indicatormix.entities.indicator import Indicator, IndicatorValueType
from indicatormix.entities.series import TrendSeries
        ...
        for idx, _ in enumerate(indicator.columns):
            try:
                ...
                raise KeyError(
                    ...
                    f"columns {func_result.columns.tolist()}"
...
def populate_with_ranges(
    ...
    args = get_function_arguments(indicator, dataframe)
    ...
    #     return populate(state, indicator_name, dataframe)
    ...
    if indicator.type == IndicatorValueType.PATTERN:
        # pattern indicators do not have optimizable parameters
        return populate(state, indicator_name, dataframe)
    if not optimizable_params:
    ...
    for value in parameter.range:
        ...
        dataframe = extract_columns_from_func(
            state, indicator, func_result, dataframe.copy(), "_" + str(value)
        )
    return dataframe
...
def populate(state: State, indicator_name: str, dataframe: DataFrame) -> DataFrame:
    indicator = state.indicator_depot.get(indicator_name)
    func_args = get_function_arguments(indicator, dataframe)
    ...
    dataframe = extract_columns_from_func(state, indicator, func_result, dataframe)
    return dataframe


def get_or_create_sma_series(
    state: State, series1: im_series.Series, dataframe: DataFrame
) -> TrendSeries:
    new_series = TrendSeries(base_series=series1)
    if new_series.series_name not in dataframe.columns:
        series = series1.get(state, dataframe)
        dataframe[new_series.series_name] = ta.SMA(series, timeperiod=4)
    return new_series
