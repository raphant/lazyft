...
class Gumbo3T(IMBaseAdvancedOptimizerStrategy):
    # region IndicatorMix
    # region Indicators
    buy_params_normal = [
    ...
    #
    # allows trailing buy uptrend for buy signals containing 'up' in buy_tag
    # enable force_buy (without trailing (cant write buy last_candle['buy'] = 1 to the dataframe), looping inside cte
    # would work but looks stupid
    ...
    def trailing_buy(self, pair, reinit=False):
        ...
        if reinit or not "trailing_buy" in self.custom_info_trail_buy[pair]:
            self.custom_info_trail_buy[pair][
                "trailing_buy"
            ] = self.init_trailing_buy_dict.copy()
    ...
    def trailing_sell(self, pair, reinit=False):
        ...
        if reinit or not "trailing_sell" in self.custom_info_trail_sell[pair]:
            self.custom_info_trail_sell[pair][
                "trailing_sell"
            ] = self.init_trailing_sell_dict.copy()
    ...
    def current_trailing_buy_profit_ratio(
    ...
    def current_trailing_sell_profit_ratio(
        self, pair: str, current_price: float
    ) -> float:
        trailing_sell = self.trailing_sell(pair)
    ...
    def trailing_buy_offset(self, dataframe, pair: str, current_price: float):
        ...
        current_trailing_profit_ratio = self.current_trailing_buy_profit_ratio(
            pair, current_price
        )
        last_candle = dataframe.iloc[-1]
        adapt = (last_candle["perc_norm"]).round(5)
        default_offset = 0.0075 * (1 + adapt)  # NOTE: default_offset 0.0045 <--> 0.009
        ...
        if trailing_duration.total_seconds() > self.trailing_expire_seconds:
            ...
            ("up" in str(last_candle["buy_tag"]) or self.trailing_buy_uptrend_enabled)
            and (
                trailing_duration.total_seconds() < self.trailing_expire_seconds_uptrend
            )
            and (
                current_trailing_profit_ratio < (-1 * self.min_uptrend_trailing_profit)
            )
        ):
            logger.info(f"uptrend buy")
    ...
    def trailing_sell_offset(self, dataframe, pair: str, current_price: float):
        ...
        last_candle = dataframe.iloc[-1]
        adapt = (last_candle["perc_norm"]).round(5)
        default_offset = 0.0075 * (1 + adapt)  # NOTE: default_offset 0.003 <--> 0.006
        ...
        if trailing_duration.total_seconds() > self.trailing_sell_expire_seconds:
            ...
            and (
                trailing_duration.total_seconds()
                < self.trailing_sell_expire_seconds_downtrend
            )
            and (
                current_trailing_sell_profit_ratio
                < (-1 * self.max_downtrend_trailing_profit)
            )
        ):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["perc"] = (
            (dataframe["high"] - dataframe["low"]) / dataframe["low"] * 100
        )
                ...
                if len(dataframe) >= 1:
                    ...
                    if trailing_buy["allow_trailing"]:
                        if not trailing_buy["trailing_buy_order_started"] and (
                            ...
                            trailing_buy["trailing_buy_order_uplimit"] = last_candle[
                                "close"
                            ]
                            ...
                            logger.info(
                                f'start trailing buy for {pair} at {last_candle["close"]}'
                            )
                            ...
                            if trailing_buy_offset == "forcebuy":
                                ...
                                ratio = "%.2f" % (
                                    (
                                        self.current_trailing_buy_profit_ratio(
                                            pair, current_price
                                        )
                                    )
                                    * 100
                                ...
                                logger.info(
                                    f'STOP trailing buy for {pair} because "trailing buy offset" returned None'
                                )

                            elif (
                                current_price
                                < trailing_buy["trailing_buy_order_uplimit"]
                            ):
                                # update uplimit
                    ...
                    elif str(last_candle["buy_tag"]) == "nan":
                        val = True
                    else:
    ...
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        val = super().confirm_trade_exit(
            pair, trade, order_type, amount, rate, time_in_force, sell_reason, **kwargs
        )
        ...
        if len(dataframe) >= 1:
            ...
            if (
                ...
                and (last_candle["sell"] == 0)
                and (not self.trailing_on_custom_sell)
            ):
                return val
            ...
            if val:
                if self.trailing_sell_order_enabled and self.config[
                    "runmode"
                ].value in (
                    "live",
                    "dry_run",
                    ...
                    current_price = rate
                    trailing_sell = self.trailing_sell(pair)
                    trailing_sell_offset = self.trailing_sell_offset(
                        dataframe, pair, current_price
                    )
                    ...
                    if not trailing_sell["allow_sell_trailing"]:
                        logger.info(
                            f"Manually triggering 'allow_SELL_trailing' to True for {pair} because of {sell_reason} and start *SELL* trailing"
                        )
                        trailing_sell["allow_sell_trailing"] = True

                    if trailing_sell["allow_sell_trailing"]:
                        if not trailing_sell["trailing_sell_order_started"] and (
                            last_candle["sell"] != 0
                        ):
                            ...
                            trailing_sell[
                                "trailing_sell_order_downlimit"
                            ] = last_candle["close"]
                            trailing_sell["start_trailing_sell_price"] = last_candle[
                                "close"
                            ]
                            trailing_sell["exit_tag"] = exit_tag
                            trailing_sell["start_trailing_time"] = datetime.now(
                                timezone.utc
                            )
                            ...
                            logger.info(
                                f'start trailing sell for {pair} at {last_candle["close"]}'
                            )
                            ...
                            if trailing_sell_offset == "forcesell":
                                ...
                                ratio = "%.2f" % (
                                    (
                                        self.current_trailing_sell_profit_ratio(
                                            pair, current_price
                                        )
                                    )
                                    * 100
                                ...
                                logger.info(
                                    f'STOP trailing sell for {pair} because "trailing sell offset" returned None'
                                )

                            elif (
                                current_price
                                > trailing_sell["trailing_sell_order_downlimit"]
                            ):
                                # update downlimit
                                old_downlimit = trailing_sell[
                                    "trailing_sell_order_downlimit"
                                ]
                                self.custom_info_trail_sell[pair]["trailing_sell"][
                                    "trailing_sell_order_downlimit"
                                ] = max(
                                ...
                                self.trailing_sell_info(pair, current_price)
                                logger.info(
                                    f'update trailing sell for {pair} at {old_downlimit} -> {self.custom_info_trail_sell[pair]["trailing_sell"]["trailing_sell_order_downlimit"]}'
                                )

                            elif current_price > (
                                trailing_sell["start_trailing_sell_price"]
                                * (1 - self.trailing_sell_max_sell)
                            ):
                                ...
                                self.trailing_sell_info(pair, current_price)
                                logger.info(
                                    f"current price ({current_price}) < downlimit ({trailing_sell['trailing_sell_order_downlimit']}) but higher than starting price ({(trailing_sell['start_trailing_sell_price'] * (1 + self.trailing_sell_max_sell))}). OK for {pair} ({ratio} %)"
                                )

                            elif current_price < (
                                trailing_sell["start_trailing_sell_price"]
                                * (1 - self.trailing_sell_max_stop)
                            ):
                                # stop trailing, sell fast, price too low
                                val = True
                                self.trailing_sell_info(pair, current_price)
                                logger.info(
                                    f"STOP trailing sell for {pair} because of the price is much lower than starting price * {1 + self.trailing_sell_max_stop}"
                                )
                            else:
                                # uplimit > current_price > max_price, continue trailing and wait for the price to go down
                                self.trailing_sell_info(pair, current_price)
        ...
        if self.trailing_buy_order_enabled and self.config["runmode"].value in (
            ...
            if last_candle["buy"] == 1:
                if not trailing_buy["trailing_buy_order_started"]:
                    ...
                    if not open_trades:
                        logger.info(
                            f"Setting 'allow_trailing' to True for {metadata['pair']} and start buy trailing!!!"
                        )
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if (
            self.trailing_buy_order_enabled
            and self.abort_trailing_when_sell_signal_triggered
            and self.config["runmode"].value in ("live", "dry_run")
        ):
            last_candle = dataframe.iloc[-1].squeeze()
            if last_candle["sell"] != 0:
                ...
                if trailing_buy["trailing_buy_order_started"]:
                    logger.info(
                        f"Sell signal for {metadata['pair']} is triggered!!! Abort trailing"
                    )
                    self.trailing_buy(metadata["pair"], reinit=True)

        if self.trailing_sell_order_enabled and self.config["runmode"].value in (
            "live",
            "dry_run",
        ):
            last_candle = dataframe.iloc[-1].squeeze()
            ...
            if last_candle["sell"] != 0:
                if not trailing_sell["trailing_sell_order_started"]:
                    ...
                    if open_trades:
                        logger.info(
                            f"Setting 'allow_SELL_trailing' to True for {metadata['pair']} and start *SELL* trailing"
                        )
                        # self.custom_info_trail_buy[metadata['pair']]['trailing_buy']['allow_trailing'] = True
                        trailing_sell["allow_sell_trailing"] = True
                        initial_sell_tag = (
                            last_candle["exit_tag"]
                            if "exit_tag" in last_candle
                            else "sell signal"
                        )
                        dataframe.loc[
                            :, "exit_tag"
                        ] = f"{initial_sell_tag} (start trail price {last_candle['close']})"
            else:
                if trailing_sell["trailing_sell_order_started"]:
                    logger.info(
                        f"Continue trailing for {metadata['pair']}. Manually trigger sell signal!"
                    )
