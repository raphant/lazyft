...
class MacheteV8b(IStrategy):
    ...
    # buy signal
    buy_should_use_get_buy_signal_awesome_macd = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_awesome_macd"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_adx_momentum = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_adx_momentum"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_adx_smas = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_adx_smas"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_asdts_rockwelltrading = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_asdts_rockwelltrading"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_averages_strategy = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_averages_strategy"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_fisher_hull = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_fisher_hull"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_gettin_moist = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_gettin_moist"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_hlhb = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_hlhb"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_macd_strategy_crossed = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_macd_strategy_crossed"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_macd_strategy = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_macd_strategy"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_pmax = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_pmax"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_quickie = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_quickie"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_scalp = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_scalp"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_simple = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_simple"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_strategy001 = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_strategy001"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_technical_example_strategy = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_technical_example_strategy"],
        space="buy",
        optimize=True,
    )
    buy_should_use_get_buy_signal_tema_rsi_strategy = CategoricalParameter(
        [True, False],
        default=buy_params["buy_should_use_get_buy_signal_tema_rsi_strategy"],
        space="buy",
        optimize=True,
    )
    ...
    droi_trend_type = CategoricalParameter(
        ["rmi", "ssl", "candle", "any"],
        default=sell_params["droi_trend_type"],
        space="sell",
        optimize=True,
    )
    droi_pullback = CategoricalParameter(
        [True, False], default=sell_params["droi_pullback"], space="sell", optimize=True
    )
    droi_pullback_amount = DecimalParameter(
        0.005, 0.02, default=sell_params["droi_pullback_amount"], space="sell"
    )
    droi_pullback_respect_table = CategoricalParameter(
        [True, False],
        default=sell_params["droi_pullback_respect_table"],
        space="sell",
        optimize=True,
    )
    ...
    cstp_trailing_only_offset_is_reached = DecimalParameter(
        0.01,
        0.06,
        default=sell_params["cstp_trailing_only_offset_is_reached"],
        space="sell",
    )
    cstp_trailing_stop_profit_devider = IntParameter(
        2, 4, default=sell_params["cstp_trailing_stop_profit_devider"], space="sell"
    )
    cstp_trailing_max_stoploss = DecimalParameter(
        0.02, 0.08, default=sell_params["cstp_trailing_max_stoploss"], space="sell"
    )
    cstp_bb_trailing_input = CategoricalParameter(
        [
            "bb_lowerband_trend",
            "bb_lowerband_trend_inf",
            "bb_lowerband_neutral",
            "bb_lowerband_neutral_inf",
            "bb_upperband_neutral_inf",
        ],
        default=sell_params["cstp_bb_trailing_input"],
        space="sell",
        optimize=True,
    )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.dp.runmode.value in ("backtest", "hyperopt"):
            self.custom_trade_info[metadata["pair"]]["roc_inf"] = (
                dataframe[["date", "roc_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["atr_inf"] = (
                dataframe[["date", "atr_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["sroc_inf"] = (
                dataframe[["date", "sroc_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["ssl-dir_inf"] = (
                dataframe[["date", "ssl-dir_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["rmi-up-trend_inf"] = (
                dataframe[["date", "rmi-up-trend_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["candle-up-trend_inf"] = (
                dataframe[["date", "candle-up-trend_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["bb_lowerband_trend_inf"] = (
                dataframe[["date", "bb_lowerband_trend_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["bb_lowerband_trend_inf"] = (
                dataframe[["date", "bb_lowerband_trend_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["bb_lowerband_neutral_inf"] = (
                dataframe[["date", "bb_lowerband_neutral_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["bb_lowerband_neutral_inf"] = (
                dataframe[["date", "bb_lowerband_neutral_inf"]].copy().set_index("date")
            )
            self.custom_trade_info[metadata["pair"]]["bb_upperband_neutral_inf"] = (
                dataframe[["date", "bb_upperband_neutral_inf"]].copy().set_index("date")
            )
    ...
    def get_market_condition_indicators(
        ...
        dataframe["future_green"] = (
            ichimoku["cloud_green"].shift(displacement).fillna(0).astype("int") * 2
        )
        ...
        dataframe["up_trend"] = np.where(
            dataframe["upper"].rolling(5, min_periods=1).sum() != 0, 1, 0
        )
        dataframe["dn_trend"] = np.where(
            dataframe["lower"].rolling(5, min_periods=1).sum() != 0, 1, 0
        )
    ...
    def get_custom_stoploss_indicators(
        ...
        bollinger_trend = qtpylib.bollinger_bands(
    ...
    def get_buy_signal_pmax(self, dataframe: DataFrame):
        signal = (self.buy_should_use_get_buy_signal_pmax.value == True) & (
