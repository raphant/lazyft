...
def pmax(df, period, multiplier, length, MAtype, src):
    ...
    if MAtype == 1:
        mavalue = ta_old.EMA(masrc, timeperiod=length)
    elif MAtype == 2:
        mavalue = ta.DEMA(masrc, period=length)
    elif MAtype == 3:
        mavalue = ta.T3(masrc, period=length)
    elif MAtype == 4:
        mavalue = ta.SMA(masrc, period=length)
    elif MAtype == 5:
        mavalue = VIDYA(df, length=length)
    elif MAtype == 6:
        mavalue = ta.TEMA(masrc, period=length)
    elif MAtype == 7:
        mavalue = ta.WMA(df, period=length)
    elif MAtype == 8:
