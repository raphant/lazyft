...
class Apollo11(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["s2_ema_xxl_off"] = (
            s2_ema_xxl_value - s2_ema_xxl_value * self.s2_fib_lower_value
        )
        ...
        dataframe["fastMA"] = ta.EMA(
            dataframe["volume"] * dataframe["close"], 12
        ) / ta.EMA(dataframe["volume"], 12)
        dataframe["slowMA"] = ta.EMA(
            dataframe["volume"] * dataframe["close"], 26
        ) / ta.EMA(dataframe["volume"], 26)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_signal_1:
            ...
            dataframe.loc[
                reduce(lambda x, y: x & y, conditions), ["buy", "buy_tag"]
            ] = (
                1,
                "buy_signal_1",
            )
...
class Apollo11Rest(BaseRestStrategy, Apollo11):
    backtest_days = 180
    hyperopt_days = 180
    hyperopt_epochs = 40
    min_avg_profit = 0.01
