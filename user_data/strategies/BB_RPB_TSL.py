...
from technical.indicators import RMI, ichimoku, zema
...
class BB_RPB_TSL(IStrategy):
    ...
    is_optimize_dip = sh.get_space("dip")
    ...
    is_optimize_break = sh.get_space("break")
    ...
    is_optimize_local_uptrend = sh.get_space("local_uptrend")
    ...
    is_optimize_local_dip = sh.get_space("local_dip")
    ...
    buy_ema_high_local_dip = DecimalParameter(
        0.90, 1.2, default=0.942, optimize=is_optimize_local_dip
    )
    ...
    is_optimize_ewo = sh.get_space("ewo")
    ...
    is_optimize_ewo_2 = sh.get_space("ewo_2")
    ...
    buy_ema_low_2 = DecimalParameter(
        0.90, 1.2, default=0.970, optimize=is_optimize_ewo_2
    )
    buy_ema_high_2 = DecimalParameter(
        0.90, 1.2, default=1.087, optimize=is_optimize_ewo_2
    )
    buy_ewo_high_2 = DecimalParameter(2, 12, default=4.179, optimize=is_optimize_ewo_2)

    is_optimize_r_deadfish = sh.get_space("r_deadfish")
    buy_r_deadfish_ema = DecimalParameter(
        0.90, 1.2, default=1.087, optimize=is_optimize_r_deadfish
    )
    buy_r_deadfish_bb_width = DecimalParameter(
        0.03, 0.75, default=0.05, optimize=is_optimize_r_deadfish
    )
    buy_r_deadfish_bb_factor = DecimalParameter(
        0.90, 1.2, default=1.0, optimize=is_optimize_r_deadfish
    )
    buy_r_deadfish_volume_factor = DecimalParameter(
        1, 2.5, default=1.0, optimize=is_optimize_r_deadfish
    )

    is_optimize_r_deadfish_protection = sh.get_space("r_deadfish_protection")
    buy_r_deadfish_cti = DecimalParameter(
        -0.6, -0.0, default=-0.5, optimize=is_optimize_r_deadfish_protection
    )
    buy_r_deadfish_r14 = DecimalParameter(
        -60, -44, default=-60, optimize=is_optimize_r_deadfish_protection
    )

    is_optimize_clucha = sh.get_space("clucha")
    buy_clucha_bbdelta_close = DecimalParameter(
        0.01, 0.05, default=0.02206, optimize=is_optimize_clucha
    )
    buy_clucha_bbdelta_tail = DecimalParameter(
        0.7, 1.2, default=1.02515, optimize=is_optimize_clucha
    )
    ...
    is_optimize_cofi = sh.get_space("cofi")
    ...
    is_optimize_cofi_protection = sh.get_space("cofi_protection")
    buy_cofi_cti = DecimalParameter(
        -0.9, -0.0, default=-0.5, optimize=is_optimize_cofi_protection
    )
    buy_cofi_r14 = DecimalParameter(
        -100, -44, default=-60, optimize=is_optimize_cofi_protection
    )

    is_optimize_gumbo = sh.get_space("gumbo")
    buy_gumbo_ema = DecimalParameter(0.9, 1.2, default=0.97, optimize=is_optimize_gumbo)
    buy_gumbo_ewo_low = DecimalParameter(
        -12.0, 5, default=-5.585, optimize=is_optimize_gumbo
    )

    is_optimize_gumbo_protection = sh.get_space("gumbo_protection")
    buy_gumbo_cti = DecimalParameter(
        -0.9, -0.0, default=-0.5, optimize=is_optimize_gumbo_protection
    )
    buy_gumbo_r14 = DecimalParameter(
        -100, -44, default=-60, optimize=is_optimize_gumbo_protection
    )

    is_optimize_sqzmom_protection = sh.get_space("sqzmom_protection")
    buy_sqzmom_ema = DecimalParameter(
        0.9, 1.2, default=0.97, optimize=is_optimize_sqzmom_protection
    )
    buy_sqzmom_ewo = DecimalParameter(
        -12, 12, default=0, optimize=is_optimize_sqzmom_protection
    )
    buy_sqzmom_r14 = DecimalParameter(
        -100, -22, default=-50, optimize=is_optimize_sqzmom_protection
    )

    is_optimize_nfix_39 = sh.get_space("nfix_39")
    buy_nfix_39_ema = DecimalParameter(
        0.9, 1.2, default=0.97, optimize=is_optimize_nfix_39
    )

    is_optimize_nfix_49_protection = sh.get_space("nfix_49_protection")
    buy_nfix_49_cti = DecimalParameter(
        -0.9, -0.0, default=-0.5, optimize=is_optimize_nfix_49_protection
    )
    buy_nfix_49_r14 = DecimalParameter(
        -100, -44, default=-60, optimize=is_optimize_nfix_49_protection
    )

    is_optimize_btc_safe = sh.get_space("btc_safe")
    ...
    is_optimize_check = sh.get_space("check")
    ...
    is_optimize_slip = sh.get_space("slip")
    ...
    is_optimize_sell_stoploss = sh.get_space("sell_stoploss")
    sell_cmf = DecimalParameter(
        -0.4, 0.0, default=0.0, optimize=is_optimize_sell_stoploss
    )
    sell_ema_close_delta = DecimalParameter(
        0.022, 0.027, default=0.024, optimize=is_optimize_sell_stoploss
    )
    sell_ema = DecimalParameter(
        0.97, 0.99, default=0.987, optimize=is_optimize_sell_stoploss
    )

    is_optimize_deadfish = sh.get_space("deadfish")
    ...
    sell_deadfish_volume_factor = DecimalParameter(
        1, 2.5, default=1.0, optimize=is_optimize_deadfish
    )

    is_optimize_bleeding = sh.get_space("bleeding")
    sell_bleeding_cti = DecimalParameter(
        -0.9, -0.0, default=-0.5, optimize=is_optimize_bleeding
    )
    sell_bleeding_r14 = DecimalParameter(
        -100, -44, default=-60, optimize=is_optimize_bleeding
    )
    sell_bleeding_volume_factor = DecimalParameter(
        1, 2.5, default=1.0, optimize=is_optimize_bleeding
    )

    is_optimize_cti_r = sh.get_space("cti_r")
    ...
    def informative_1h_indicators(
        ...
        # nfi 37
        informative_1h["hl_pct_change_5"] = range_percent_change(
            informative_1h, "HL", 5
        )
    ...
    def custom_sell(
        ...
        **kwargs,
        ...
        if (
            ...
            and (
                last_candle["close"]
                > last_candle["bb_middleband2"] * self.sell_deadfish_bb_factor.value
            )
            and (
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        bollinger2_40 = qtpylib.bollinger_bands(
            ha_typical_price(dataframe), window=40, stds=2
        )
        ...
        dataframe["kc_upperband_20_2"] = (
            dataframe["sma_20"] + dataframe["range_ma_20"] * 2
        )
        dataframe["kc_lowerband_20_2"] = (
            dataframe["sma_20"] - dataframe["range_ma_20"] * 2
        )
        ...
        dataframe["avg_val_20"] = (
            dataframe["avg_hh_ll_20"] + dataframe["avg_close_20"]
        ) / 2
        dataframe["linreg_val_20"] = ta.LINEARREG(
            dataframe["close"] - dataframe["avg_val_20"], 20, 0
        )
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_sqzOff = (dataframe["bb_lowerband2"] < dataframe["kc_lowerband_28_1"]) & (
        ...
        is_r_deadfish = (  # reverse deadfish
            (
                dataframe["ema_100"]
                < dataframe["ema_200"] * self.buy_r_deadfish_ema.value
            )
            & (dataframe["bb_width"] > self.buy_r_deadfish_bb_width.value)
            & (
                dataframe["close"]
                < dataframe["bb_middleband2"] * self.buy_r_deadfish_bb_factor.value
            )
            & (
        ...
        is_clucHA = (dataframe["rocr_1h"] > self.buy_clucha_rocr_1h.value) & (
            (dataframe["bb_lowerband2_40"].shift() > 0)
            & (
                dataframe["bb_delta_cluc"]
                > dataframe["ha_close"] * self.buy_clucha_bbdelta_close.value
            )
            & (
                dataframe["ha_closedelta"]
        ...
        is_sqzmom = (  # Modified from squeezeMomentum, credit goes to original author @LazyBear of TradingView
            (is_sqzOff)
            & (
                dataframe["linreg_val_20"].shift(2)
                > dataframe["linreg_val_20"].shift(1)
            )
        ...
        if conditions:
            dataframe.loc[
                is_additional_check & reduce(lambda x, y: x | y, conditions), "buy"
            ] = 1
