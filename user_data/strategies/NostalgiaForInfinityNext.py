...
class NostalgiaForInfinityNext(IStrategy):
    ...
    def dynamic_volume_list(self):
        ...
        if sorted(self.top_traded_coins["current_whitelist"]) != sorted(
        ...
        if self.top_traded_coins["updated"] == False:
            ...
            self.top_traded_coins["current_whitelist"].insert(
                0,
                self.top_traded_coins["current_whitelist"].pop(
                    self.top_traded_coins["current_whitelist"].index(
                        f"BTC/{self.config['stake_currency']}"
                    )
                ),
            )
            ...
            for coin_pair in self.top_traded_coins["current_whitelist"]:
                ...
                pair_dataframe = self.dp.get_pair_dataframe(
                    pair=coin_pair, timeframe=self.info_timeframe_1d
                )
                ...
                pair_dataframe[coin] = pair_dataframe["volume"] * qtpylib.typical_price(
                    pair_dataframe
                )
                ...
                self.top_traded_coins["dataframe"] = self.top_traded_coins[
                    "dataframe"
                ].merge(pair_dataframe, on="date", how="left")
            ...
            column_names = [
                f"Coin #{i}" for i in range(1, self.top_traded_coins["list_length"] + 1)
            ]
            self.top_traded_coins["dataframe"][column_names] = self.top_traded_coins[
                "dataframe"
            ].apply(
                lambda x: x.nlargest(self.top_traded_coins["list_length"]).index.values,
                axis=1,
                result_type="expand",
            )
            self.top_traded_coins["dataframe"].drop(
                columns=[
                    col
                    for col in self.top_traded_coins["dataframe"]
            ...
            self.top_traded_coins["dataframe"].insert(
                loc=0, column="date", value=pair_dates
            )
            self.top_traded_coins["dataframe"].set_index("date")
    ...
    def is_top_coin(self, coin_pair, row_data) -> bool:
        return (
            coin_pair.split("/")[0]
            in row_data.loc[
                "Coin #1":f"Coin #{self.top_traded_coins['list_length']}"
            ].values
        )
        ...
        if last_candle["close"] > (last_candle["res3_1d"] * 2.2):
            if 0.02 > current_profit >= 0.012:
                ...
                if (
                    ...
                    and (last_candle["r_480"] > -15.0)
                ):
                    return True, "signal_profit_pv_1_4_1"
            elif 0.06 > current_profit >= 0.05:
                if (
                    (last_candle["r_14"] > -1.2)
    ...
    def custom_sell(
        ...
        **kwargs,
        ...
        if all(c in ["48"] for c in buy_tags):
            sell, signal_name = self.sell_uptrend_mode(
                current_profit, max_profit, last_candle, previous_candle_1
            )
            if sell and (signal_name is not None):
                return f"{signal_name} ( {buy_tag} )"
        ...
        if any(
            ...
            sell, signal_name = self.sell_stoploss_atr(
                current_profit, last_candle, previous_candle_1, trade, current_time
            )
            if sell and (signal_name is not None):
                return f"{signal_name} ( {buy_tag} )"
        ...
        if any(
            ...
            sell, signal_name = self.sell_stoploss_extra(
                current_profit,
                max_profit,
                max_loss,
                last_candle,
    ...
    def informative_1d_indicators(
        ...
        informative_1d = self.dp.get_pair_dataframe(
            pair=metadata["pair"], timeframe=self.info_timeframe_1d
        )

        # Top traded coins
        if self.top_traded_coins["enabled"]:
            informative_1d = informative_1d.merge(
                self.top_traded_coins["dataframe"], on="date", how="left"
            )
            informative_1d["is_top_coin"] = informative_1d.apply(
                lambda row: self.is_top_coin(metadata["pair"], row), axis=1
            )
            column_names = [
                f"Coin #{i}" for i in range(1, self.top_traded_coins["list_length"] + 1)
            ]
        ...
        (
            informative_1d["open_sha"],
            informative_1d["close_sha"],
            informative_1d["low_sha"],
        ) = HeikinAshi(
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["cci_36_osc"] = (cci_36 / cci_36_max).where(
            cci_36 > 0, -cci_36 / cci_36_min
        )
        ...
        tok = time.perf_counter()
        log.debug(
            f"[{metadata['pair']}] normal_tf_indicators took: {tok - tik:0.4f} seconds."
        )
    ...
    def base_tf_btc_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        tok = time.perf_counter()
        log.debug(
            f"[{metadata['pair']}] base_tf_btc_indicators took: {tok - tik:0.4f} seconds."
        )
    ...
    def info_tf_btc_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        tok = time.perf_counter()
        log.debug(
            f"[{metadata['pair']}] info_tf_btc_indicators took: {tok - tik:0.4f} seconds."
        )
    ...
    def daily_tf_btc_indicators(
        ...
        tok = time.perf_counter()
        log.debug(
            f"[{metadata['pair']}] daily_tf_btc_indicators took: {tok - tik:0.4f} seconds."
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.has_BTC_daily_tf:
            ...
            drop_columns = [
                f"{s}_1d" for s in ["date", "open", "high", "low", "close", "volume"]
            ]
            dataframe.drop(
        ...
        if self.has_BTC_info_tf:
            btc_info_tf = self.dp.get_pair_dataframe(
                btc_info_pair, self.info_timeframe_1h
            )
            ...
            dataframe = merge_informative_pair(
                ...
                btc_info_tf,
                self.timeframe,
                self.info_timeframe_1h,
                ffill=True,
            )
            drop_columns = [
                f"{s}_{self.info_timeframe_1h}"
        ...
        if self.info_timeframe_1d != "none":
            ...
            dataframe = merge_informative_pair(
                ...
                informative_1d,
                self.timeframe,
                self.info_timeframe_1d,
                ffill=True,
            )
            drop_columns = [
                f"{s}_{self.info_timeframe_1d}"
                for s in ["date", "open", "high", "low", "close", "volume"]
            ]
            dataframe.drop(
                columns=dataframe.columns.intersection(drop_columns), inplace=True
        ...
        if self.res_timeframe != "none":
            ...
            dataframe.rename(
                columns=lambda s: f"{s}_{self.res_timeframe}"
                if "resample_" in s
                else s,
                inplace=True,
            )
            ...
            if self.buy_params[f"buy_condition_{index}_enable"]:
                ...
                if global_buy_protection_params["close_over_pivot_type"] != "none":
                    item_buy_protection_list.append(
                        dataframe["close"]
                        > dataframe[
                            f"{global_buy_protection_params['close_over_pivot_type']}_1d"
                        ]
                ...
                if global_buy_protection_params["close_under_pivot_type"] != "none":
                    item_buy_protection_list.append(
                        dataframe["close"]
                        < dataframe[
                            f"{global_buy_protection_params['close_under_pivot_type']}_1d"
                        ]
                ...
                if index == 1:
                    ...
                    item_buy_logic.append(
                        (
                            (dataframe["close"] - dataframe["open"].rolling(12).min())
                            / dataframe["open"].rolling(12).min()
                        )
                        > self.buy_1_min_inc
                    )
                ...
                # Condition #3
                    ...
                    item_buy_logic.append(
                        dataframe["bb40_2_delta"].gt(
                            dataframe["close"] * self.buy_3_bb40_bbdelta_close
                        )
                    )
                    item_buy_logic.append(
                        dataframe["closedelta"].gt(
                            dataframe["close"] * self.buy_3_bb40_closedelta_close
                        )
                    )
                    item_buy_logic.append(
                        dataframe["tail"].lt(
                            dataframe["bb40_2_delta"] * self.buy_3_bb40_tail_bbdelta
                        )
                    )
                    item_buy_logic.append(
                    ...
                    item_buy_logic.append(
                        dataframe["cci_36_osc"] > self.buy_3_cci_36_osc_min
                    )
                    item_buy_logic.append(dataframe["crsi_1h"] > self.buy_3_crsi_1h_min)
                    item_buy_logic.append(
                        dataframe["r_480_1h"] > self.buy_3_r_480_1h_min
                    )
                ...
                # Condition #4
                    ...
                    item_buy_logic.append(
                        dataframe["close"]
                        < self.buy_4_bb20_close_bblowerband * dataframe["bb20_2_low"]
                    )
                    item_buy_logic.append(
                        dataframe["volume"]
                        < (
                            dataframe["volume_mean_30"].shift(1)
                            * self.buy_4_bb20_volume
                        )
                    )
                ...
                # Condition #5
                    ...
                    item_buy_logic.append(
                        dataframe["r_14"].shift(1) < self.buy_5_r_14_max
                    )
                ...
                # Condition #9
                    ...
                    # Logic
                    item_buy_logic.append(
                        dataframe["close"] < dataframe["ema_20"] * self.buy_9_ma_offset
                    )
                    item_buy_logic.append(
                        dataframe["close"]
                        < dataframe["bb20_2_low"] * self.buy_9_bb_offset
                    )
                    ...
                    item_buy_logic.append(
                        dataframe["rsi_14_1h"] > self.buy_9_rsi_1h_min
                    )
                    item_buy_logic.append(
                        dataframe["rsi_14_1h"] < self.buy_9_rsi_1h_max
                    )
                ...
                # Condition #11
                    ...
                    item_buy_logic.append(
                        (
                            (dataframe["close"] - dataframe["open"].rolling(6).min())
                            / dataframe["open"].rolling(6).min()
                        )
                        > self.buy_11_min_inc
                    )
                    item_buy_logic.append(
                    ...
                    item_buy_logic.append(
                        dataframe["r_480_1h"] < self.buy_11_r_480_1h_max
                    )
                    item_buy_logic.append(
                        dataframe["crsi_1h"] > self.buy_11_crsi_1h_min
                    )
                ...
                # Condition #12
                    ...
                    # Logic
                    item_buy_logic.append(
                        dataframe["close"] < dataframe["sma_30"] * self.buy_12_ma_offset
                    )
                ...
                # Condition #13
                    ...
                    item_buy_logic.append(
                        dataframe["crsi_1h"] > self.buy_13_crsi_1h_min
                    )
                ...
                # Condition #14
                    ...
                    item_buy_logic.append(
                        (dataframe["ema_26"] - dataframe["ema_12"])
                        > (dataframe["open"] * self.buy_14_ema_open_mult)
                    )
                    item_buy_logic.append(
                        (dataframe["ema_26"].shift() - dataframe["ema_12"].shift())
                        > (dataframe["open"] / 100)
                    ...
                    item_buy_logic.append(
                        dataframe["close"]
                        < (dataframe["bb20_2_low"] * self.buy_14_bb_offset)
                    )
                    item_buy_logic.append(
                        dataframe["close"] < dataframe["ema_20"] * self.buy_14_ma_offset
                    )
                ...
                # Condition #15
                    ...
                    item_buy_logic.append(
                        (dataframe["ema_26"] - dataframe["ema_12"])
                        > (dataframe["open"] * self.buy_15_ema_open_mult)
                    )
                    item_buy_logic.append(
                        (dataframe["ema_26"].shift() - dataframe["ema_12"].shift())
                        > (dataframe["open"] / 100)
                    ...
                    item_buy_logic.append(
                        dataframe["close"] < dataframe["ema_20"] * self.buy_15_ma_offset
                    )
                ...
                # Condition #17
                    ...
                    item_buy_logic.append(
                        dataframe["crsi_1h"] > self.buy_17_crsi_1h_min
                    )
                    item_buy_logic.append(
                        dataframe["volume"]
                ...
                # Condition #19
                    ...
                    item_buy_logic.append(
                        dataframe["rsi_14_1h"] > self.buy_19_rsi_1h_min
                    )
                ...
                # Condition #28
                    ...
                    item_buy_logic.append(
                        dataframe["cti"].shift(1) < self.buy_28_cti_max
                    )
                ...
                # Condition #32 - Quick mode buy
                    ...
                    item_buy_logic.append(
                        dataframe["crsi_1h"] > self.buy_32_crsi_1h_min
                    )
                ...
                # Condition #34 - Quick mode buy
                    ...
                    item_buy_logic.append(
                        dataframe["crsi_1h"] > self.buy_34_crsi_1h_min
                    )
                    item_buy_logic.append(
                        dataframe["volume"]
                ...
                # Condition #36 - PMAX1 buy
                    ...
                    item_buy_logic.append(
                        dataframe["crsi_1h"] > self.buy_36_crsi_1h_min
                    )
                ...
                # Condition #37 - PMAX2 buy
                    ...
                    item_buy_logic.append(
                        (
                            (dataframe["close_1h"].shift(12) - dataframe["close_1h"])
                            / dataframe["close_1h"]
                        )
                ...
                # Condition #43
                    ...
                    item_buy_logic.append(
                        dataframe["r_480_1h"] > self.buy_43_r_480_1h_min
                    )
                ...
                # Condition #48 - Uptrend mode
                    ...
                    item_buy_logic.append(
                        dataframe["r_480_1h"] > dataframe["r_480_1h"].shift(12)
                    )
                    item_buy_logic.append(dataframe["cti_1h"] > self.buy_48_cti_1h_min)
                    item_buy_logic.append(
                        dataframe["crsi_1h"] > self.buy_48_crsi_1h_min
                    )
                    item_buy_logic.append(
                        dataframe["cti"].shift(1).rolling(12).min() < -0.5
                    )
                    item_buy_logic.append(
                        dataframe["cti"].shift(1).rolling(12).max() < 0.0
                    )
