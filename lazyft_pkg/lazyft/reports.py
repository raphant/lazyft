...
class BacktestReportBrowser:
    ...
    def get_hashes(self, strategy: str):
        return [p.get('hash') for p in self.backtest_data.get(strategy, {})]
    ...
    def get_all_backtest_results(self):
        frames = []
        for s in self.backtest_data:
            df = self.get_backtest_results(s)
    ...
    def get_backtest_by_hash(self, strategy: str, hash: str) -> Optional[dict]:
        for p in self.backtest_data[strategy]:
            if p.get('hash') == hash:
                return p


class HyperoptReportsBrowser(UserDict):
    def __init__(self):
        data = rapidjson.loads(PARAMS_FILE.read_text())
        super().__init__(data)
...
if __name__ == '__main__':
    print(BacktestReportBrowser().get_hashes('TestBinH'))
