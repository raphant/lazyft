...
from quicktools.regex import EPOCH_LINE_REGEX
...
class HyperoptRunner(runner.Runner):
    ...
    def live_output(self):
        ...
        Extractor(self.message_queue, table).start()
    ...
    def on_finished(self, cmd, success, exit_code):
        ...
        if self.strategy.path_to_delete.exists():
            self.strategy.path_to_delete.unlink()
        if not success:
            logger.error(self.output)
