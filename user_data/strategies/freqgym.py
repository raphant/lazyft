    ...
    def _take_action(self, action):
        ...
        if action == 1:
            if self.trade is None:
                ...
                self.trades.append({"step": self.index, "type": 'buy', "total": self.status.open})

                logger.debug(
                    "{} - Backtesting emulates creation of new trade: {}.".format(
        ...
        if action == 2:
            if self.trade is not None:
                ...
                self.trades.append({"step": self.index, "type": 'sell', "total": self.status.open})
