...
class BigZ04_TSL4(IStrategy):
    ...
    buy_bb20_close_bblowerband_safe_1 = DecimalParameter(
        0.950, 1.050, default=0.989, decimals=3, space="buy", optimize=True, load=True
    )
    buy_bb20_close_bblowerband_safe_2 = DecimalParameter(
        0.700, 1.100, default=0.982, decimals=2, space="buy", optimize=True, load=True
    )
    ...
    buy_volume_drop_1 = DecimalParameter(
        1, 10, default=3.8, space="buy", decimals=1, optimize=True, load=True
    )
    buy_volume_drop_2 = DecimalParameter(
        1, 10, default=3, space="buy", decimals=1, optimize=True, load=True
    )
    buy_volume_drop_3 = DecimalParameter(
        1, 10, default=2.7, space="buy", decimals=1, optimize=True, load=True
    )

    buy_rsi_1h_0 = DecimalParameter(
        55.0, 85.0, default=71.0, space="buy", decimals=1, optimize=True, load=True
    )
    buy_rsi_1h_1a = DecimalParameter(
        65.0, 78.0, default=69.0, space="buy", decimals=1, optimize=True, load=True
    )
    ...
    buy_rsi_1h_5 = DecimalParameter(
        10.0, 60.0, default=39.0, space="buy", decimals=1, optimize=True, load=True
    )

    buy_rsi_0 = DecimalParameter(
        10.0, 40.0, default=30.0, space="buy", decimals=1, optimize=True, load=True
    )
    ...
    buy_dip_0 = DecimalParameter(
        1.015, 1.040, default=1.024, space="buy", decimals=3, optimize=True, load=True
    )

    # hyperopt parameters for custom_stoploss()
    trade_time = IntParameter(
        25, 65, default=35, space="sell", optimize=True, load=True
    )
    rsi_1h_val = IntParameter(
        25, 45, default=32, space="sell", optimize=True, load=True
    )
    narrow_stop = DecimalParameter(
        1.005, 1.030, default=1.020, space="sell", decimals=3, optimize=True, load=True
    )
    wide_stop = DecimalParameter(
        1.010, 1.045, default=1.035, space="sell", decimals=3, optimize=True, load=True
    )

    # hyperopt parameters for SMAOffsetProtectOptV1 sell signal
    base_nb_candles_sell = IntParameter(
        5, 80, default=49, space="sell", optimize=True, load=True
    )
    high_offset = DecimalParameter(
        0.99, 1.1, default=1.006, space="sell", optimize=True, load=True
    )
    ...
    pPF_2 = DecimalParameter(
        0.040, 0.100, default=0.080, decimals=3, space="sell", optimize=True, load=True
    )
    pSL_2 = DecimalParameter(
        0.020, 0.070, default=0.040, decimals=3, space="sell", optimize=True, load=True
    )
    ...
    def informative_1h_indicators(
        ...
        # EMA
        try:
            informative_1h["ema_50"] = ta.SMA(informative_1h, period=50)
        except Exception as e:
            logger.info("inf frame is %s", informative_1h)
        informative_1h["ema_200"] = ta.SMA(informative_1h, period=200)
        # RSI
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # EMA
        dataframe["ema_200"] = ta.SMA(dataframe, period=200)

        dataframe["ema_26"] = ta.EMA(dataframe, period=26)
        dataframe["ema_12"] = ta.EMA(dataframe, period=12)

        # MACD
        dataframe["macd"], dataframe["signal"], dataframe["hist"] = ta_old.MACD(
            dataframe["close"], fastperiod=12, slowperiod=26, signalperiod=9
        )

        # SMA
        dataframe["sma_5"] = ta.EMA(dataframe, period=5)
