...
class InverseV2(IStrategy):
    ...
    buy_fisher_length = IntParameter(
        low=13, high=55, default=34, space="buy", optimize=True, load=True
    )
    buy_fisher_cci_1 = DecimalParameter(
        low=-0.6,
        high=-0.3,
        decimals=2,
        default=-0.5,
    ...
    buy_fisher_cci_2 = DecimalParameter(
        low=0.3,
        high=0.6,
        decimals=2,
        default=0.5,
    ...
    sell_fisher_cci_1 = DecimalParameter(
        low=0.3,
        high=0.6,
        decimals=2,
        default=0.5,
    ...
    sell_fisher_cci_2 = DecimalParameter(
        low=-0.6,
        high=-0.3,
        decimals=2,
        default=-0.5,
    ...
    def informative_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_p = self.dp.get_pair_dataframe(
    ...
    def informative_btc_indicators(
        self, dataframe: DataFrame, metadata: dict
    ) -> DataFrame:
        assert self.dp, "DataProvider is required for multiple timeframes."

        informative_btc = self.dp.get_pair_dataframe(
            pair="BTC/USDT", timeframe=self.info_timeframe
        )
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for cci_length in self.buy_fisher_length.range:
            ...
            dataframe[f'fisher_cci_{cci_length}'] = (numpy.exp(2 * wmacci) - 1) / (
                numpy.exp(2 * wmacci) + 1
            )
        ...
        dataframe['di_up'] = ta.PLUS_DI(dataframe, timeperiod=3) > ta.MINUS_DI(
            dataframe, timeperiod=3
        )
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (
                    (
                        qtpylib.crossed_above(
                            dataframe[f'fisher_cci_{self.buy_fisher_length.value}'],
                            self.buy_fisher_cci_1.value,
                        )
                    )
                    | (
                        (
                ...
                & (
                    dataframe[f'ssl_up_{self.info_timeframe}']
                    > dataframe[f'ssl_down_{self.info_timeframe}']
                )
                & (dataframe['ema_50'] > dataframe['ema_200'])
                & (
                    dataframe[f'ema_50_{self.info_timeframe}']
                    > dataframe[f'ema_100_{self.info_timeframe}']
                )
                & (
                    dataframe[f'ema_50_{self.info_timeframe}']
                    > dataframe[f'ema_200_{self.info_timeframe}']
                )
            ...
            (
                (
                    (
                        qtpylib.crossed_below(
                            dataframe[f'fisher_cci_{self.buy_fisher_length.value}'],
                            self.sell_fisher_cci_1.value,
                        )
                    )
                    | (
                        qtpylib.crossed_below(
                            dataframe[f'fisher_cci_{self.buy_fisher_length.value}'],
                            self.sell_fisher_cci_2.value,
                        )
                    )
                )
                & (dataframe['volume'] > 0)  # Make sure Volume is not 0
