...
class LongShortRangeTradingMachetesV1(IStrategy):
    ...
    droi_pullback_amount = DecimalParameter(
        0.005, 0.02, default=0.005, space="sell", optimize=True
    )
    ...
    cstp_threshold = DecimalParameter(
        -0.05, 0, default=-0.03, space="sell", optimize=True
    )
    ...
    cstp_trailing_stop_positive_offset = DecimalParameter(
        0.005, 0.06, default=0.01, space="sell", optimize=True
    )
    cstp_trailing_stop_profit_devider = IntParameter(
        2, 4, default=2, space="sell", optimize=True
    )
    cstp_trailing_max_stoploss = DecimalParameter(
        0.02, 0.08, default=0.02, space="sell", optimize=True
    )
    cstp_trailing_enabled = CategoricalParameter(
        [True, False], default=True, space="sell", optimize=True
    )
    ...
    def on_populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe = self.get_base_pair_dataframe(
            dataframe, base_pair_name, self.timeframe_long
        )
        # dataframe = self.get_base_pair_dataframe(dataframe, base_pair_name, self.timeframe_medium)
        dataframe = self.get_base_pair_dataframe(
            dataframe, base_pair_name, self.timeframe
        )
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[((dataframe["entry_signal"] == 1)), "buy"] = 1
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[((dataframe["exit_signal"] == 1)), "sell"] = 1
    ...
    def get_indicators(self, dataframe):
        ...
        dataframe["overbought_price"] = dataframe["overbought_price"].replace(
        ...
        dataframe["oversold_price"] = dataframe["oversold_price"].replace(
            to_replace=0, method="ffill"
        )
    ...
    def get_base_pair_dataframe(
        self,
        dataframe,
        base_pair_name,
        timeframe_str,
        smooth_list=None,
        candels_per_timeframe=2,
    ):
        ...
        ignore_columns = ["date"]
        base_pair_dataframe.rename(
            columns=lambda s: "base_" + s if (not s in ignore_columns) else s,
            inplace=True,
        )
        dataframe = merge_informative_pair(
            dataframe, base_pair_dataframe, self.timeframe, timeframe_str, ffill=True
        )

        if smooth_list != None:
            for indicator_key in smooth_list:
                dataframe[f"base_{indicator_key}_{timeframe_str}"] = ta.SMA(
                    dataframe[f"base_{indicator_key}_{timeframe_str}"],
                    timeperiod=candels_per_timeframe,
                )
            ...
            (
                (
                    qtpylib.crossed_above(
                        dataframe[f"base_close_{self.timeframe}"],
                        dataframe[f"base_oversold_price_{self.timeframe_long}"],
                    )
                )
            )
            .fillna(0)
        ...
        dataframe["has_exit_signal"] = (
            (
                (
                    qtpylib.crossed_above(
                        dataframe[f"base_close_{self.timeframe}"],
                        dataframe[f"base_overbought_price_{self.timeframe_long}"],
                    )
                )
            )
            .fillna(0)
    ...
    def init_trade_flow_info(self, pair):
        ...
        self.set_trade_flow_state(
            self.TF_STAND_BY_WAITING_FOR_MARKET_CONDITION_STATE, pair
        )
    ...
    def calc_trade_flow(self, dataframe, metadata):
        ...
        for row_df in zip(
            ...
            if self.is_trade_flow_state(
                self.TF_STAND_BY_WAITING_FOR_MARKET_CONDITION_STATE, pair
            ):
                if row_df[0] == 1:
                    self.set_trade_flow_state(
                        self.TF_ENTERED_MARKET_CONDITION_WAITING_FOR_CONFIRMATION_STATE,
                        pair,
                    )
                else:
                    continue
            ...
            if self.is_trade_flow_state(
                self.TF_ENTERED_MARKET_CONDITION_WAITING_FOR_CONFIRMATION_STATE, pair
            ):
                if row_df[1] == 1:
                    if row_df[2] == 1:
                        self.set_trade_flow_state(
                            self.TF_MARKET_CONDITION_CONFIRMED_WAITING_FOR_ENTRY_SIGNAL_STATE,
                            pair,
                        )
                    else:
                        continue
            ...
            if self.is_trade_flow_state(
                self.TF_MARKET_CONDITION_CONFIRMED_WAITING_FOR_ENTRY_SIGNAL_STATE, pair
            ):
                if row_df[1] == 1:
                    if row_df[2] == 1:
                        if row_df[3] == 1:
                            self.set_trade_flow_state(
                                self.TF_ENTRY_SIGNAL_FOUND_STATE, pair
                            )
                        else:
                            continue
                    else:
            ...
            if self.is_trade_flow_state(self.TF_ENTRY_SIGNAL_FOUND_STATE, pair):
                ...
                self.set_trade_flow_state(
                    self.TF_ENTRY_SIGNAL_FOUND_WAITING_FOR_EXIT_SIGNAL_STATE, pair
                )
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        if current_profit < self.cstp_threshold.value:
            ...
            if (
                self.cstp_trailing_enabled.value == True
                and current_profit >= self.cstp_trailing_stop_positive_offset.value
            ):
                desired_stoploss = (
                    current_profit / self.cstp_trailing_stop_profit_devider.value
                )
                new_stoploss = max(
                    min(desired_stoploss, self.cstp_trailing_max_stoploss.value), 0.025
                )
