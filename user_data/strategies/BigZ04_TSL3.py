...
class BigZ04_TSL3(IStrategy):
    ...
    trade_time = IntParameter(
        25, 65, default=35, space="sell", optimize=False, load=True
    )
    rsi_1h_val = IntParameter(
        25, 45, default=32, space="sell", optimize=False, load=True
    )
    narrow_stop = DecimalParameter(
        1.005, 1.030, default=1.020, space="sell", decimals=3, optimize=False, load=True
    )
    wide_stop = DecimalParameter(
        1.010, 1.045, default=1.035, space="sell", decimals=3, optimize=False, load=True
    )

    # hyperopt parameters for SMAOffsetProtectOptV1 sell signal
    base_nb_candles_sell = IntParameter(
        5, 80, default=49, space="sell", optimize=False, load=True
    )
    high_offset = DecimalParameter(
        0.99, 1.1, default=1.006, space="sell", optimize=False, load=True
    )
