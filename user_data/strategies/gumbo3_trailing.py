...
class Gumbo3T(IMBaseAdvancedOptimizerStrategy):
    ...
    debug_mode = True
    trailing_buy_max_stop = (
        0.02  # stop trailing buy if current_price > starting_price * (1+trailing_buy_max_stop)
    )
    ...
    trailing_sell_max_stop = (
        0.02  # stop trailing sell if current_price < starting_price * (1+trailing_buy_max_stop)
    )
    ...
    def current_trailing_buy_profit_ratio(self, pair: str, current_price: float) -> float:
        ...
        if trailing_buy['trailing_buy_order_started']:
            return (trailing_buy['start_trailing_price'] - current_price) / trailing_buy[
    ...
    def current_trailing_sell_profit_ratio(self, pair: str, current_price: float) -> float:
        ...
        if trailing_sell['trailing_sell_order_started']:
            return (current_price - trailing_sell['start_trailing_sell_price']) / trailing_sell[
    ...
    def trailing_sell_offset(self, dataframe, pair: str, current_price: float):
        ...
        if trailing_duration.total_seconds() > self.trailing_sell_expire_seconds:
            ...
            and (trailing_duration.total_seconds() < self.trailing_sell_expire_seconds_downtrend)
            and (current_trailing_sell_profit_ratio < (-1 * self.max_downtrend_trailing_profit))
        ):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['perc_norm'] = (dataframe['perc'] - dataframe['perc'].rolling(50).min()) / (
                    ...
                    if trailing_buy['allow_trailing']:
                        if not trailing_buy['trailing_buy_order_started'] and (
                            ...
                            if trailing_buy_offset == 'forcebuy':
                                ...
                                val = True
                                ratio = "%.2f" % (
                                    (self.current_trailing_buy_profit_ratio(pair, current_price))
                                    * 100
                                )
                                self.trailing_buy_info(pair, current_price)
                                logger.info(
                ...
                if self.trailing_sell_order_enabled and self.config['runmode'].value in (
                    ...
                    if trailing_sell['allow_sell_trailing']:
                        if not trailing_sell['trailing_sell_order_started'] and (
                            ...
                            if exit_tag == '':
                                exit_tag = (
                                    f'{sell_reason} (start trail price {last_candle["close"]})'
                                )
                            ...
                            if trailing_sell_offset == 'forcesell':
                                ...
                                val = True
                                ratio = "%.2f" % (
                                    (self.current_trailing_sell_profit_ratio(pair, current_price))
                                    * 100
                                )
                                self.trailing_sell_info(pair, current_price)
            ...
            if last_candle['buy'] == 1:
                if not trailing_buy['trailing_buy_order_started']:
                    ...
                    if not open_trades:
                        ...
                        initial_buy_tag = (
                            last_candle['buy_tag'] if 'buy_tag' in last_candle else 'buy signal'
                        )
                        dataframe.loc[
                            :, 'buy_tag'
            ...
            if last_candle['sell'] != 0:
                if not trailing_sell['trailing_sell_order_started']:
                    ...
                    if open_trades:
                        ...
                        trailing_sell['allow_sell_trailing'] = True
                        initial_sell_tag = (
                            last_candle['exit_tag'] if 'exit_tag' in last_candle else 'sell signal'
                        )
                        dataframe.loc[
                            :, 'exit_tag'
